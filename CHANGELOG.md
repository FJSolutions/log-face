...
- A new, global override to disable all logging, log normally, or override the log-level set by individual loggers.
...
- The main `log-face.ts` file that the `index.ts` simply exports.
	...
	- Currently the scope of the project is to add a coloured label to every log entry with a timestamp and a message,
